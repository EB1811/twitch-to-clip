...
def dlAndCreate(id: str, outputName: str = None, twitchdlArgs: list = None, ffmpegArgs: list = None):
    print("\n---Downloading---\n")
    (title, user, game, outputVidName) = downloadTwitch(
        id, outputName=outputName, twitchdlArgs=twitchdlArgs)

    print("\n---Creating Video---\n")
    createShort(f"{outputVidName}.mp4",
                f"{outputVidName}.mp4", ffmpegArgs)
...
def getIdFromUrl(url: str):
    ...
    return (url.split("/videos/") if isVideo(url) else url.split("/clip/"))[1].split("?")[0]
